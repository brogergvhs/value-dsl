...
other commands use the same parser/analysis surface where needed. The LSP server
also consumes `analysis.Result`, but caches it per open document and uses
fallback navigation data from the last good index when the current text is
temporarily invalid.
